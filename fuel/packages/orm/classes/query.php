...
class Query
{
	...
	{
		$insert = \DB::insert(call_user_func($this->model.'::table'), array_keys($this->values))
			->values(array_values($this->values));
		
		// TODO:PostgreSQLのシーケンスID対応
		$m = new $this->model;
		$insert->seq_name = isset($m->seq_name) ? $m->seq_name : call_user_func($this->model.'::table').'_id_seq';
		$res = $insert->execute($this->write_connection);
